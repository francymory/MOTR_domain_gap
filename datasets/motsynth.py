...
"""
MOTSYNTH dataset which returns image_id for evaluation.
"""
...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, dataset2transform):
        ...
        self.label_files = [(x.replace('images', 'labels_with_ids').replace('rgb', 'img1').replace('.png', '.txt').replace('.jpg', '.txt'))
    ...
    def _pre_single_frame(self, idx: int):
        ...
        if 'MOTSYNTH' in img_path:
            targets['dataset'] = 'MOTSYNTH'
        else:
...
def make_transforms_for_motsynth(image_set, args=None):
...
def build_dataset2transform(args, image_set):
    motsynth_train = make_transforms_for_motsynth('train', args)
    motsynth_test = make_transforms_for_motsynth('val', args)

    dataset2transform_train = {'MOTSYNTH': motsynth_train}
    dataset2transform_val = {'MOTSYNTH': motsynth_test}
    if image_set == 'train':
